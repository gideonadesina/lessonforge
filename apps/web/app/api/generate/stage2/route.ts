  ...
  try {
    ...
    const resp = await client.responses.create({
      ...
      input: [
        ...
        { role: "user", content: buildStage2Prompt(meta, existing) },
      ],
      temperature: 0.2,
    ...
    const stage2 = normalizeStage2(parsed.data);
    const validationError = validateStage2(stage2);
    if (validationError) {
    ...
    const nextData = {
      ...
      lessonPlan: {
        ...(existing.lessonPlan ?? {}),
        evaluation: stage2.lessonPlan.evaluation,
        exitTicket: stage2.lessonPlan.exitTicket,
        assignment: stage2.lessonPlan.assignment,
      },
      subjectEnrichment: {
        ...(existing.subjectEnrichment ?? {}),
        ...stage2.subjectEnrichment,
      },
      quiz: stage2.quiz,
      meta: {
        ...(existing.meta ?? {}),
    ...
    if (updateErr) {
      console.error("[generate:stage2] Failed to update lesson:", updateErr.message);
      return NextResponse.json(
    ...
    const message = error instanceof Error ? error.message : "Stage 2 generation failed.";
    console.error("[generate:stage2] Error:", error);
    return NextResponse.json({ ok: false, error: "stage2_failed", message }, { status: 500 });
  }
}
