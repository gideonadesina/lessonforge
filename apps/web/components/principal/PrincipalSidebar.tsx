...
const NAV_SECTIONS: NavSection[] = [
  {
    title: "Main",
    items: [
      { label: "Dashboard", href: "/principal", view: "dashboard", icon: LayoutDashboard },
      { label: "Teachers", href: "/principal?view=teachers", view: "teachers", icon: Users },
      { label: "Generate", href: "/principal?view=generate", view: "generate", icon: Sparkles },
      { label: "Library", href: "/principal?view=library", view: "library", icon: Library },
    ],
  },
  {
    title: "Management",
    items: [
      { label: "School Workspace", href: "/principal?view=workspace", view: "workspace", icon: Building2 },
      { label: "Planning", href: "/principal?view=planning", view: "planning", icon: CalendarDays },
      { label: "Analytics", href: "/principal?view=analytics", view: "analytics", icon: ChartColumn },
    ],
  },
  {
    title: "Billing",
    items: [
      { label: "Subscription / Billing", href: "/principal?view=billing", view: "billing", icon: CreditCard },
      { label: "Slot Management", href: "/principal?view=slots", view: "slots", icon: UserRoundPlus },
    ],
  },
  {
    title: "System",
    items: [
      { label: "Notifications", href: "/principal?view=notifications", view: "notifications", icon: Bell },
      { label: "Settings", href: "/principal?view=settings", view: "settings", icon: Settings },
    ],
  },
];

function isActive(view: string, currentView: string) {
  if (view === "dashboard") {
          ...
          <div key={section.title}>
            ...
            <div className={collapsed ? "space-y-1" : "mt-1 space-y-1"}>
              {section.items.map((item) => {
                const activeItem = isActive(item.view, currentView);
...
export default function PrincipalSidebar({
  ...
  const currentView = pathname === "/principal" ? view : "dashboard";
